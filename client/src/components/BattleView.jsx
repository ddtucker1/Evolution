...
export default function BattleView({
  ...
  const renderHandPanel = (hand, isPlayer, { interactive = false } = {}) => {
    if (!hand?.length) return null;

    return (
      <div className={`zone-hand-panel${isPlayer ? ' zone-hand-panel-player' : ' zone-hand-panel-opponent'}`}>
        {interactive && (
          <>
    ...
    return (
      <>
        <div className={`zone-battle-row${isPlayer ? ' zone-battle-row-player' : ' zone-battle-row-opponent'}`}>
          {isPlayer && showDeckAndHand && renderHandPanel(hand, true, { interactive: true })}
          <div className={`field-formation card-panel${isPlayer ? ' player-formation' : ' opponent-formation'}`}>
            {isPlayer ? (
              <>
                {edgeSlots}
                {mainGrid}
          ...
          {!isPlayer && showDeckAndHand && renderHandPanel(hand, false)}
        </div>
        {magicActions && (
